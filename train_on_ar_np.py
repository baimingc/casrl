...
for ep in range(test_epoch):
    ...
    #             print('task: ', task_idx,'step: ', i, 'acc_reward: ', acc_reward, 'violation_rate: ', sum(V)/len(V))
        ...
        if ep % save_every == save_every -1:
            ...
            torch.save(model.model.state_dict(), './log/ar_np_{}_{}.pth'.format(log_name, ep))
            torch.save(acc_R, './log/ar_np_{}_reward.pth'.format(log_name))
            print('model saved at'+log_name)
    model.reset()
    model.train()
    time_end = time.time()
    print('episodic time:', time_end-time_start)