'''
@Author: Mengdi Xu, Wenhao Ding, Zuxin Liu, Jiacheng Zhu
@Email:
@Date: 2020-03-17 12:32:22
@LastEditTime: 2020-05-14 13:47:38
@Description:
'''
...
class DPMixture:
    def __init__(self, model, args, nn_config):
        ...
        self.meta_model_path = "meta_model.pth"
    ...
    # NOTICE: Use this function to decide the cluster id
    #   comment: For NP, there are two way
    #       1: Self-fit , train the NP for some iterations on (coming unknow data)
    #       2: Meta-model, pre-train a meta-model
    #       3: Global Prior, can think it for later
    def svi(self, dataset, prior_model, model_config):
        # TODO: What is the size of the input dataset?
        #   comment, it seems to be a list
        ...
        # print('type(prior_model) =', type(prior_model))

        # calculate \rho_{n+1, 1:k} - [k, 1]
        n_comps = len(self.comps)
        rho_old = [self.comps[k].likelihood(dataset) for k in range(n_comps)]

        # NOTICE: Given a new prior
        #   The prior can be a model trained using small iterations

        # Initialize a new model component
        new_comp_prior = self.component_model(model_config, dataset)
        new_comp_prior.fit_prior(dataset=dataset, logger=False)

        # new_comp_prior = prior_model    # TODO: Need to be updated
        ...
        rho = rho_old+rho_new
        rho = rho/np.sum(rho)
        print('After normalization, rho = ', rho)
        ...
        if cluster_idx == n_comps:
            ...
            new_comp = self.component_model(model_config, dataset)
            # print(' len(dataset) =', len(dataset))
            # print('type(dataset) =', type(dataset))
            # print('dataset =', dataset)

            # NOTICE: The data is already initialed
            # for i in range(len(dataset)) :
            #     # print('data =', data)
            #     print('i =', i)
            #     print('dataset[i] =', dataset[i])
            #     new_comp.add_data_point(dataset[i])
            print('fitting the new model')
            # print('new_comp.dataset =', new_comp.dataset)
            new_comp.fit(logger=False)
            ...
            # NOTICE: Why load meta_model here?
            # TODO: Need to modify here
            #   Here we first try not to use "meta model"
            # self.comps[cluster_idx].load_model(self.meta_model_path)
            self.comps[cluster_idx].fit(logger=False)
        else:
            logger.error('Index exceeds the length of components!')

        # NOTICE: The self merge is commented
