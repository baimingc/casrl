...
class NP_component_test():
    def __init__(self):
        self.iteration = 1500
        # Intereting,
        self.NP = CUDA(NeuralProcessModel(x_dim=5,
                                     y_dim=4,
    ...
    def fit(self, dataset, batch_size=1, print_info=False):
        ...
        for it in range(self.iteration):
            ...
            context_x_i = CUDA(context_x_i.view((1, -1, 5)))
            context_y_i = CUDA(context_y_i.view((1, -1, 4)))
            target_x_i = CUDA(target_x_i.view((1, -1, 5)))
            target_y_i = CUDA(target_y_i.view((1, -1, 4)))
    ...
    def likelihood_np(self, test_dataset):
        ...
        x = CUDA(x.view((1, -1, 5)))
        y = CUDA(y.view((1, -1, 4)))
        test_x = CUDA(test_x.view((1, -1, 5)))
        test_y = CUDA(test_y.view((1, -1, 4)))
...
def list_2_torch(data_list):
    ...
    ctt_x_array = np.zeros((data_len, 5))
    ctt_y_array = np.zeros((data_len, 4))

    for i, data in enumerate(data_list):
        # print('i =', i)
        cluster_id, current_env, action, next_env = data
        ctt_x_array[i, 0:4] = current_env
        ctt_x_array[i, 4] = action
...
if __name__ == '__main__':
    ...
    test_observation_num = 5  # (number of data in a episode)
