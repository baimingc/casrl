...
if __name__ == '__main__':
    ...
    config = load_config('config/config_swingup_adaptation.yml')
...
#     model.reset()
    ...
    for ep in range(1):
        for task_idx in range(len(task)):
            env = task[task_idx]
            task_steps = 0
            for epi in range(test_episode[task_idx]):
                acc_reward = 0
                obs = env.reset()
                ...
                if done:
                    ...
                    if not prior_safety:
                        path = './misc/log/np_adaptation_' + log_name.strftime("%d-%H-%M") + '.npy'
                    else:
                        path = './misc/log/np_safe_adaptation_' + log_name.strftime("%d-%H-%M") + '.npy'
