...
class Sampler(BaseSampler):
    ...
    def __init__(
        ...
        # if use parallel, the envs will be sampled simultaneously 
    ...
    def obtain_samples(self, log=False, log_prefix='', random=False):
        ...
        while n_samples < self.total_samples:
            # execute policy
            t = time.time()
            # use random policy at the first iteration
            if random:
                actions = np.stack([self.env.action_space.sample() for _ in range(num_envs)], axis=0)
                agent_infos = {}
            # use dynamics_model policy, it will create adaptive models to output the action
            else:
                a_bs = self.adapt_batch_size
                if a_bs is not None and len(running_paths[0]['observations']) > a_bs + 1:
                    adapt_obs = [np.stack(running_paths[idx]['observations'][-a_bs - 1:-1]) for idx in range(num_envs)]
                    adapt_act = [np.stack(running_paths[idx]['actions'][-a_bs-1:-1]) for idx in range(num_envs)]
                    adapt_next_obs = [np.stack(running_paths[idx]['observations'][-a_bs:]) for idx in range(num_envs)]
            ...
            # step environments
            # [Q] does not change the environment? Yes, the env will not be changed during the training
            t = time.time()
