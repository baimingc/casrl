...
class MetaMLPDynamicsModel(Serializable):
    ...
    def fit(self, obs, act, obs_next, epochs=1000, compute_normalization=True, valid_split_ratio=None, rolling_average_persitency=None):
        ...
        obs_train, act_train, delta_train, obs_test, act_test, delta_test = train_test_split(obs, act, delta, test_split_ratio=valid_split_ratio)
        ...
        num_steps_per_epoch = max(int(np.prod(self._dataset_train['obs'].shape[:2])/(self.meta_batch_size * self.batch_size * 2)), 1)
        num_steps_test = max(int(np.prod(self._dataset_test['obs'].shape[:2])/(self.meta_batch_size * self.batch_size * 2)), 1)
        ...
        for epoch in range(epochs):
            ...
            for _ in range(num_steps_per_epoch):
                ...
                pre_batch_loss, post_batch_loss, _ = sess.run(
                    [self.pre_loss, self.post_loss, self.train_op],
                    feed_dict={self.obs_ph: obs_batch, self.act_ph: act_batch, self.delta_ph: delta_batch}
                )
            ...
            for _ in range(num_steps_test):
                ...
                feed_dict = {self.obs_ph: obs_test, self.act_ph: act_test, self.delta_ph: delta_test}
    ...
    def _predict(self, obs, act):
        # use the adaptive model to predict
    ...
    # call this function to adapt the meta model to a new model
    def adapt(self, obs, act, obs_next):
        # [Q] the number of adaptive model is the same as the number of observations?
        # [A] This is designed for parallel MPC policy, which can process several environments at the same time
        ...
        sess = tf.get_default_session()
        self._adapted_param_values = sess.run(
            self._adapted_params[:self._num_adapted_models],
    ...
    def _get_batch(self, train=True):
        if train:
            ...
            obs_batch = np.concatenate([self._dataset_train['obs'][ip, ib - self.batch_size:ib + self.batch_size, :]
                for ip, ib in zip(idx_path, idx_batch)], axis=0)
            act_batch = np.concatenate([self._dataset_train['act'][ip, ib - self.batch_size:ib + self.batch_size, :]
                for ip, ib in zip(idx_path, idx_batch)], axis=0)
            delta_batch = np.concatenate([self._dataset_train['delta'][ip, ib - self.batch_size:ib + self.batch_size, :]
                for ip, ib in zip(idx_path, idx_batch)], axis=0)
        else:
            ...
            obs_batch = np.concatenate([self._dataset_test['obs'][ip, ib - self.batch_size:ib + self.batch_size, :]
                for ip, ib in zip(idx_path, idx_batch)], axis=0)
            act_batch = np.concatenate([self._dataset_test['act'][ip, ib - self.batch_size:ib + self.batch_size, :]
                for ip, ib in zip(idx_path, idx_batch)], axis=0)
            delta_batch = np.concatenate([self._dataset_test['delta'][ip, ib - self.batch_size:ib + self.batch_size, :]
                for ip, ib in zip(idx_path, idx_batch)], axis=0)
...
def train_test_split(obs, act, delta, test_split_ratio=0.2):
    ...
    return obs[idx_train, :], act[idx_train, :], delta[idx_train, :], obs[idx_test, :], act[idx_test, :], delta[idx_test, :]
