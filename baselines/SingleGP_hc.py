...
@LastEditTime: 2020-05-15 12:31:26
...
class SingleGP(object):
    ...
    def __init__(self, gp_config):
        ...
        self.trigger_training = CUDA(torch.tensor(3e-4))
    ...
    def data_process(self, data):
        if self.action_dim == 0:
            ...
            s = data[1][1:]  # dont input x into model
            a = data[2]
            s_n = data[3]
    ...
    def add_point(self, x):
        data_point = self.data_process(x)
    ...
    def predict(self, s, a):
        if self.action_dim == 0:
            test_x = CUDA(torch.Tensor(s[:, 1:self.state_dim]))
        else:
            x = np.concatenate((s[:, 1:], a), axis=1)
