...
def concat_tensor_list(tensor_list, max_buffer=None):
    if max_buffer is None:
        return np.concatenate(tensor_list, axis=0)
    else:
        return np.concatenate(tensor_list, axis=0)[-max_buffer:]


def concat_tensor_dict_list(tensor_dict_list, max_buffer=None):
    keys = list(tensor_dict_list[0].keys())
    ret = dict()
    for k in keys:
        example = tensor_dict_list[0][k]
        if isinstance(example, dict):
            v = concat_tensor_dict_list([x[k] for x in tensor_dict_list], max_buffer=max_buffer)
        else:
            v = concat_tensor_list([x[k] for x in tensor_dict_list], max_buffer=max_buffer)
        ret[k] = v
    return ret
