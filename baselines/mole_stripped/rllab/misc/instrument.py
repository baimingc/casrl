...
from mole.utils import flatten
import collections
...
def to_local_command(params, python_command="python -u", script=osp.join(config.PROJECT_PATH,
...
def to_docker_command(params, docker_image, python_command="python", script='scripts/run_experiment_lite.py',
    ...
    command_list.append(to_local_command(
        params, python_command=python_command, script=osp.join(config.DOCKER_CODE_DIR, script),
        use_gpu=use_gpu))
...
def launch_ec2(params_list, exp_prefix, docker_image, code_full_path,
               ...
               sync_s3_h5=False,
               sync_log_on_termination=True,
               periodic_sync=True, periodic_sync_interval=15, verbose=True):
    ...
    for params in params_list:
        ...
        sio.write("""
            {command}
        """.format(
            command=to_docker_command(params, docker_image, python_command=python_command, script=script,
...
def to_lab_kube_pod(
            ...
            if sync_s3_pkl:
                pre_commands.append("""
                            while /bin/true; do
                                ...
                                sleep {periodic_sync_interval}
                            done & echo sync initiated""".format(log_dir=log_dir,
    ...
    if pre_commands is not None:
        command_list.extend(pre_commands)
    command_list.append("echo \"Running in docker\"")
    # command_list.append('sleep 100000000000000')
    command_list.append(
        "%s 2>&1 | tee -a %s" % (
