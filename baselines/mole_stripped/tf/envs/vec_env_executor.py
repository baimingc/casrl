...
class VecEnvExecutor(object):
    ...
    def step(self, action_n, max_path_length, currpathlength_list=-7):
        assert len(action_n) == len(self.envs)
        all_results = [env.step(a, currpathlength) for (a, env, currpathlength) in zip(action_n, self.envs, currpathlength_list)]
