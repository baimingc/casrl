...
@LastEditTime: 2020-05-17 21:02:37
@Description:
'''
...
class NN(object):
    ...
    def __init__(self, NN_config):
        ...
        #     self.model = CUDA(torch.load_state_dict(model_config["model_path"]))
        # else:
        #     self.model = CUDA(MLP(self.input_dim, self.state_dim, model_config["hidden_dim"], model_config["hidden_size"]))
        self.model = CUDA(MLP(self.input_dim, self.state_dim, model_config["hidden_dim"], model_config["hidden_size"]))
        if model_config["load_model"]:
            print('loading',model_config["model_path"])
...
#                 print(np.shape(datas), np.shape(outputs), np.shape(labels))
