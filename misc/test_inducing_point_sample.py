...
@LastEditTime: 2020-04-15 19:36:51
@Description: Gaussian Process component for high-dimensional input and output
'''
...
class SampleGPR(gpytorch.models.ExactGP):
    ...
    def inducing_ELBO(self, Xs):
        ...
        B = CUDA(torch.tensor(1.0)) - A.diagonal()
...
class GPComponent(object):
    def __init__(self):
        ...
        self.max_inducing_point = 50
        self.trigger_induce = 299
        ...
        self.param = [
            1e-3,   # noise_covar initilize
            1e-4,   # noise_covar constraint
...
train_x = torch.linspace(0, 20, 300)
# True function is sin(2*pi*x) with Gaussian noise
train_y = torch.sin(train_x) + torch.randn(train_x.size()) * 0.01
train_data = torch.cat((train_x[:, None], train_y[:, None]), axis=1)
print(train_data.shape)

gp = GPComponent()

for i in range(len(train_x)):
    gp.add_point(train_data[i, :][None], i)
    gp.train_model()

inducing_point = gp.get_point()
inducing_x = inducing_point[:, 0:1]
inducing_y = inducing_point[:, 1:2]

f, ax = plt.subplots(1, 1, figsize=(8, 6))
ax.plot(train_x.numpy(), train_y.numpy(), 'b.')
ax.plot(inducing_x, inducing_y, 'r.', markersize=15)
plt.show()
