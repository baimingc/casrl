...
for ep in range(test_epoch):
    ...
    #             print('task: ', task_idx,'step: ', i, 'acc_reward: ', acc_reward, 'violation_rate: ', sum(V)/len(V))
        print(ep, 'acc_reward: ', acc_reward, 'success_food',info['task_success'], 'a', a2, 'distance', np.linalg.norm(obs[6:9]))
        acc_R.append(acc_reward)
        if ep % save_every == save_every -1:
            if log_name is None:
                log_name = time.strftime("%Y%m%d_%H%M%S")
            torch.save(model.model.state_dict(), './log/ar_np_roll_{}_{}.pth'.format(log_name, ep))
            torch.save(acc_R, './log/ar_np_roll_{}_reward.pth'.format(log_name))
