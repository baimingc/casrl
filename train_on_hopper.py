...
@LastEditTime: 2020-05-17 20:24:10
@Description: 
'''
...
import gym
sys.path.append('./envs/hopper-env')
import hopper_envs

from utils import plot_reward, plot_index, dumb_reward_plot, save_test_succ_rate, Noise, dumb_predict_error_plot
# use fetch slide mpccontroller
from mpc.mpc_hp import MPC
...
from baselines.NN_hc import NN
...
if __name__ == '__main__':
    ...
    config = load_config('./config/config_hopper.yml')
    ...
    action_noise = Noise(mu=np.zeros(mpc_config['Random']['action_dim']), sigma=0.02)
    ...
    while (not task_solved) and (task_epi < gym_config['task_episode']):
        ...
        for subtask_index in range(len(task)):
            for epi in range(gym_config['subtask_episode']): # each subtask contains a fixed number of episode
                ...
                if (epi+1) % test_iter == 0:
                    ...
                    print('-------------------------------------------------')
                    print('Test reward: ', acc_test_reward)
                    print('-------------------------------------------------')
                    save_test_succ_rate(acc_test_reward_list, PREFIX='Hopper')
                ...
                while not done:
                    ...
                    #print('observation', ground_truth)
                    print('state_next', _state_next[0:6])
                    print('MSE error ', mse_error)
                    #mse_error = 0
                    E.append(mse_error)
                    #print('rootz', _state_next[1])
                    ...
                    if done:
                        ...
                        dumb_predict_error_plot(path, PREFIX='Hopper', ylim=[0, 10])
                        dumb_reward_plot(path, PREFIX='Hopper', xlim=[0, 200], ylim=[0, 600], y_line1=100, y_line2=100)
        ...
        if task_epi % 1 == 0:
            ...
            if model.name in ['DPGPMM', 'SDPGPMM']:
                ...
                plot_index(predict_list, label_list, name='Cluster_Result_with_HalfCheetah_' + str(task_epi) + '.png')
            else:
